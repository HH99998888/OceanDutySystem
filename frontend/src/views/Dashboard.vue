...
const data=ref({sites:[],modules:[],abnormalSites:[]}), loading=ref(false)
...
const check=async()=>{await http.post('/sites/check');await refresh()}
const healthy=computed(()=>data.value.sites.filter(s=>s.status!=='ABNORMAL'))
onMounted(refresh)
</script>
<template><div v-loading="loading"><section class="title-row"><div><h2>监控总览</h2><p>实时掌握网站可用性和预报产品更新时间</p></div><el-button type="primary" @click="check">立即检测</el-button></section><el-alert v-for="site in data.abnormalSites" :key="site.id" type="error" :closable="false" show-icon class="alert" :title="`${site.siteName} 无法访问`" :description="`${site.errorMessage || '访问异常'} · ${site.lastCheckTime || '未检测'}`"/><h3>网站监控</h3><el-row :gutter="16"><el-col v-for="site in healthy" :key="site.id" :xs="24" :sm="12" :lg="8"><el-card class="card"><div class="card-head"><b>{{site.siteName}}</b><el-tag :type="tagType(site.status)">{{statusText(site.status)}}</el-tag></div><p>响应时间：{{site.responseTime == null ? '—' : site.responseTime + ' ms'}}</p><small>最近检测：{{site.lastCheckTime || '待检测'}}</small></el-card></el-col></el-row><h3>业务模块</h3><el-row :gutter="16"><el-col v-for="item in data.modules" :key="item.id" :xs="24" :sm="12" :lg="6"><el-card class="card"><div class="card-head"><b>{{item.moduleName}}</b><el-tag :type="tagType(item.status)">{{statusText(item.status)}}</el-tag></div><p>更新时间：{{item.updateTime || '待识别'}}</p><small>预期周期：每天 {{item.expectedTime}}</small></el-card></el-col></el-row></div></template>
